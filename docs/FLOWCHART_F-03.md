...
本文件根據 `PRD_F-03.md` 定義的功能需求，規劃了「虛擬寵物養成系統」的使用者操作動線與後端資料流。
...
sequenceDiagram
    ...
    participant Flask as Flask 後端路由
    participant DB as SQLite 資料庫
    
    User->>Browser: 點擊「餵食」按鈕
    Browser->>Flask: 發送 POST /api/pet/feed (夾帶身份驗證)
...
以下是針對虛擬寵物養成功能，初步規劃的路由與對應方法：

| 功能名稱 | 功能說明 | URL 路徑 | HTTP 方法 |
| --- | --- | --- | --- |
| 寵物專屬頁面 | 回傳包含寵物畫面的 Jinja2 網頁模板 | `/pet` | GET |
| 餵食寵物 API | 接收餵食指令，於後端計算經驗值與升級邏輯 | `/api/pet/feed` | POST |
| 取得寵物狀態 API | 供前端非同步獲取最新等級與經驗值 (更新畫面用) | `/api/pet/status` | GET |
| 管理員設定 API | 供系統管理員調整每次餵食獲得的 EXP 參數 (Nice to have) | `/admin/pet/config` | PUT/POST |
